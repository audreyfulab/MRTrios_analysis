# ============================================================
# Annotate BLCA trio results with CpG genomic location
...
raw_dir    <- "/Users/lianzuo/LZ/ResearchProject/Fulab/GDCdata/TCGA-BLCA/raw_data"
out_dir    <- "/Users/lianzuo/LZ/ResearchProject/Fulab/MRTrios_BLCA/Output_BLCA"
model_file <- "/Users/lianzuo/LZ/ResearchProject/Fulab/MRTrios_BLCA/Output_BLCA/BLCA_trio_Model_results_ALL_with_BH_fdr_qval_byLZ.txt"
...
# ============================================================
Model_BLCA <- fread(model_file, sep = "\t", data.table = FALSE)

Model_BLCA$Inferred.Model3 <- Model_BLCA$Inferred.Model.BH_fdr

n_model_rows <- nrow(Model_BLCA)

merge_Model_BLCA_df <- left_join(
  Model_BLCA, loc,
  by = c("trio_row" = "trios_row", "cna.row", "gene.row", "meth.row")
)

if (nrow(merge_Model_BLCA_df) != n_model_rows) {
  warning(sprintf(
...
# ============================================================
# final_file <- file.path(out_dir, "LZ_trio_results_Model_BLCA_ALL_with_BH_fdr_qval_with_location.txt")
# fwrite(Model_BLCA_df_2, final_file, sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)

final_file <- file.path(out_dir, "LZ_trio_results_Model_BLCA_ALL_with_BH_fdr_qval_with_location.rds")
saveRDS(Model_BLCA_df_2, final_file, compress = "xz")

cat(sprintf("Done. Final file: %s (%d rows)\n", final_file, nrow(Model_BLCA_df_2)))

## Final file: /Users/lianzuo/LZ/ResearchProject/Fulab/MRTrios_BLCA/Output_BLCA/LZ_trio_results_Model_BLCA_ALL_with_BH_fdr_qval_with_location.txt (675071 rows)
